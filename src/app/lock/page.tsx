    ...
    <section className="w-full h-screen flex overflow-hidden justify-center items-center bg-[radial-gradient(circle,_#FFC8DD,_#ffffff)]">
      <div className='w-[90%] max-w-7xl rounded-tl-[50px] rounded-br-[50px] bg-white pr-0 p-3 flex h-[95%] '>
        ...
        <div className='flex-1  p-10 flex justify-end items-center  flex-col space-y-5  h-full'>
          ...
          <h1 className='text-4xl font-bold   '>Anniversary&apos;s Lock</h1>

          <div className={`w-full transition-all duration-300 relative h-16 ${number.length === 6 ? (isSuccess ? "bg-green-100 border-green-500 text-gray-950" : "bg-red-100 border-red-500 text-gray-950 ") : "bg-gray-950 text-gray-100"} border  rounded-md overflow-hidden `}>
            <div className='flex justify-center items-center h-full space-x-2'>
              {number.length > 0 ? (
                number.map((n, i) => (
                  <span
                    key={i}
                    className=" text-2xl font-semibold text-center  animate-digit-entry"
                ...
                <span className="text-white text-2xl font-light italic opacity-50 animate-fade-in">
                  Enter your number
                </span>
              )}
            </div>
            ...
            {/* lock icon */}
            <i className='absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 transition-colors duration-300'>
          ...
          <Keyboard setNumber={setNumber} number={number} setIsSuccess={setIsSuccess} />
