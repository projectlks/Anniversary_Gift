...
// ---------------------------------------------------------
// 🌟 Sticky Note Component (Mobile Friendly UX ဖြင့် အပြည့်အစုံ) 🌟
// ---------------------------------------------------------
function StickyNote({
  note,
  isSelected,
  ...
  // စာရိုက်လက်စကို မှတ်ထားမည့် Ref (ဖုန်းမှာ အပြင်နှိပ်မိလျှင် Auto Save ဖို့)
  ...
  // အပြင်ဘက် နှိပ်လိုက်လျှင် Auto Save ပြုလုပ်မည့် အပိုင်း
  useEffect(() => {
    if (!isSelected) {
      if (isEditing) {
        setIsEditing(false);
        if (textRef.current !== note.content) {
          onContentChange(textRef.current);
        }
      }
    ...
    <motion.div
      ...
      {isSelected && !isEditing && (
        <div
          ...
          {/* ✏️ Edit ခလုတ် (Mobile အတွက်) */}
          <button
            onClick={() => setIsEditing(true)}
            className="w-8 h-8 md:w-7 md:h-7 rounded-full flex items-center justify-center text-gray-500 hover:text-blue-500 hover:bg-blue-50 transition-colors"
            title="Edit Note">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
      ...
      {/* ✅ Done Button (Mobile တွင် စာရိုက်နေစဉ် ပေါ်မည်) */}
      <AnimatePresence>
        {isEditing && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className="absolute -bottom-14 left-1/2 -translate-x-1/2 z-[70]">
            <button
              onPointerDown={(e) => {
                e.preventDefault();
                handleSaveText();
              }}
              className="bg-[#0d99ff] text-white px-6 py-2.5 rounded-full shadow-xl font-bold text-sm tracking-wide whitespace-nowrap border border-white/20 hover:bg-blue-600 transition">
              ✅ Done
            </button>
          </motion.div>
        )}
      ...
      <textarea
        ...
        className={`relative z-30 w-full h-full p-4 md:p-5 bg-transparent outline-none resize-none text-gray-800 text-sm sm:text-base md:text-lg leading-relaxed ${!isEditing ? "pointer-events-none" : "pointer-events-auto"}`}
      />

      {/* 🌟 Character Count ပြမည့် အပိုင်း */}
      ...
      <AnimatePresence>
        {isEditing && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            // right-14 (56px) သို့ ရွှေ့ထားသဖြင့် ခေါက်ထားသော ထောင့် (48px) နှင့် လွတ်သွားပါမည်။
            // စာသားများနှင့် မရောသွားစေရန် bg-white/50 ဖြင့် နောက်ခံအလင်းလေး ထည့်ပေးထားပါသည်။
            className={`absolute bottom-3 right-14 text-xs font-medium z-40 px-2 py-0.5 rounded-md bg-white/40 backdrop-blur-sm shadow-sm border border-white/50 ${
              text.length >= 150 ? "text-red-600 font-bold" : "text-gray-700"
            }`}>
            {text.length} / 150
          </motion.div>
        )}
